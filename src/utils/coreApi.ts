...
class ApiClient {
    ...
    */
    ...
    putStorageItems(id: string, collection: string, key: string, value: any) {
        return this.authFetch(`/storages/${encodeURIComponent(id)}/${this.platform}/${this.organization}/${this.company}/${this.app}/${encodeURIComponent(collection)}/${encodeURIComponent(key)}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ value }),
        });
    ...
    async getData(
        ...
        const res = await this.authFetch(
            `/storages/${encodeURIComponent(userId)}/${this.platform}/${this.organization}/${this.company}/${this.app}/${encodeURIComponent(collection)}/${keyString}`
        );
        const data =
            res?.[this.platform]?.[this.organization]?.[this.company]?.[this.app]?.[collection] || [];
    ...
    setData(
        ...
        return this.authFetch(
            `/storages/${encodeURIComponent(userId)}/${this.platform}/${this.organization}/${this.company}/${this.app}/${encodeURIComponent(collection)}/${encodeURIComponent(keys.join(","))}`,
            {
                method: 'PUT',
                headers: {
    ...
    async uploadFile(id: string, key: string, file: File) {
        ...
        const res = await fetch(`${this.API_BASE_URL}/storages/upload`, {
            method: "POST",
            headers: {
